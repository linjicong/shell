...
if ! [ -x "$(command -v curl)" ]; then >>audit.txt
 ifconfig | grep --color "\([0-9]\{1,3\}\.\)\{3\}[0-9]\{1,3\}" >>audit.txt
else
  echo "ifconfig 命令不存在" >>audit.txt
fi
...
echo "----------------------------------------------------------------" >>audit.txt
grep TMOUT /etc/profile /etc/bashrc > /dev/null|| echo "未设置登录超时限制，请设置，设置方法：在/etc/profile或者/etc/bashrc里面添加TMOUT=600参数" >>audit.txt
echo "----------------------------------------------------------------" >>audit.txt
...
if [ $? -eq 0 ];then
  echo "已设置远程root不能登陆，符合要求" >>audit.txt
else
  echo "未设置远程root不能登陆，不符合要求，建议/etc/ssh/sshd_config添加PermitRootLogin no" >>audit.txt
fi
echo "----------------------------------------------------------------" >>audit.txt
...
ls -ltr /bin/ls /bin/login /etc/passwd /bin/ps /usr/bin/top /etc/shadow|awk '{print "文件名："$9"  ""最后修改时间："$8" "$6" "$7}' >>audit.txt
